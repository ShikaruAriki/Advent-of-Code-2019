...
def passwords_meet_criteria(start, end):
    count = 0
    for password in range(start, end + 1):
        if _password_parser(password):
            count += 1

    return count


def _password_parser(password):
...
def passwords_meet_criteria_2(start, end):
    count = 0
    for password in range(start, end + 1):
        if _password_parser_2(password):
            count += 1

    return count


def _password_parser_2(password):
    password = str(password)
    ...
    for char in password:
        ...
        if before == now:
            ...
            adjacent = adjacent or count == 1
            count = 0

        before = now

    return adjacent or count == 1
