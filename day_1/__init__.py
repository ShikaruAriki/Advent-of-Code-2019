...
def get_total_fuel():
    ...
    for line in util.read("day_1/input.txt"):
        module_mass = int(line)

        total_fuel += int(module_mass // 3) - 2
...
def get_total_fuel_correct():
    total_fuel = 0

    for line in util.read("day_1/input.txt"):
        module_mass = int(line)

        total_fuel += _get_fuel_for_mass(module_mass)
...
def _get_fuel_for_mass(mass):
    ...
    return fuel + _get_fuel_for_mass(fuel)
