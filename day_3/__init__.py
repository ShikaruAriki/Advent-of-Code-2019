...
def get_manhattan_distance_from_central_point():
    return sorted(_get_intersection(), key=lambda o: o['manhattan_distance'])[0]['manhattan_distance']


def _get_intersection():
    file_read = util.read("day_3/input.txt")

    wire_1 = file_read[0].split(",")
    wire_2 = file_read[1].split(",")

    wire_1_point = _get_wire_points(wire_1)
    wire_2_point = _get_wire_points(wire_2)

    intersection = []

    for x, y_items in wire_1_point.items():
        for y, total_step in y_items.items():
            if x in wire_2_point:
                if y in wire_2_point[x]:
                    intersection.append({
                        'manhattan_distance': abs(x) + abs(y),
                        'total_step': total_step + wire_2_point[x][y]
                    })
...
def _get_wire_points(wire):
    ...
    for movement in wire:
        ...
        for i in range(step):
            ...
            point[position] = operation(point[position], 1)

            _add_point(wire_point, point, total_step)

    return wire_point


def _add_point(wire_point, point, total_step):
    if point[0] in wire_point:
        if point[1] not in wire_point[point[0]]:
            wire_point[point[0]][point[1]] = total_step
...
def get_minimized_signal_delay_from_central_point():
    return sorted(_get_intersection(), key=lambda o: o['total_step'])[0]['total_step']
