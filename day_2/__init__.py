...
def get_first_code_of_restored_gravity_assist_test_program():
    program_restored = _restore_gravity_assist_test_program()

    return program_restored[0]


def _restore_gravity_assist_test_program():
    program_to_restore = util.read("day_2/input.txt")[0]

    program_code = program_to_restore.split(",")

    for i in range(len(program_code)):
        program_code[i] = int(program_code[i])

    program_code[1] = 12
    program_code[2] = 2

    return _working_computer(program_code)


def _working_computer(program_code):
    i = 0
    halt = False

    while i < len(program_code) and not halt:
        operation = OPERATIONS.get(program_code[i], None)

        if operation:
            index_first_registry = program_code[i + 1]
            index_second_registry = program_code[i + 2]
            index_result_registry = program_code[i + 3]

            program_code[index_result_registry] = operation(
                program_code[index_first_registry],
                program_code[index_second_registry]
            )
...
def get_input_of_restored_gravity_assist_program():
    program_to_restore = util.read("day_2/input.txt")[0]

    memory = program_to_restore.split(",")
    ...
    for noun in range(100):
        for verb in range(100):
            ...
            program_code = _working_computer(program_code)
